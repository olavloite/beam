...
public class MinimalWordCount
{
	...
	public static void main(String[] args)
	{
		...
		// of Shakespeare.
		PCollection<KV<String, String>> groupedByFirstLetter = p
				.apply(TextIO.read().from("gs://apache-beam-samples/shakespeare/*"))

				// Concept #2: Apply a ParDo transform to our PCollection of
				// text lines. This ParDo invokes a
				// DoFn (defined in-line) on each element that tokenizes the
				// text line into individual words.
				// The ParDo returns a PCollection<String>, where each element
				// is an individual word in
				...
				// transform returns a new PCollection of key/value pairs, where
				// each key represents a unique
				// word in the text. The associated value is the occurrence
				// count for that word.
				.apply(Count.<String> perElement())

				// Apply a MapElements transform that formats our PCollection of
				// word counts into a printable
		...
		PCollection<KV<String, Iterable<String>>> grouped = groupedByFirstLetter
				.apply(GroupByKey.<String, String> create());

		grouped.apply(ParDo.of(new GetValues())).apply(JdbcIO.<String> writeIterable()
