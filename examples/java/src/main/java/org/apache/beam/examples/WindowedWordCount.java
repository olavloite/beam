...
import org.apache.beam.examples.WordCount.FirstLetterAsKey;
import org.apache.beam.examples.WordCount.GetValues;
 ...
 * successively more detailed 'word count' examples. First take a look at
 * {@link MinimalWordCount}, {@link WordCount}, and {@link DebuggingWordCount}.
 *
 * <p>
 * Basic concepts, also in the MinimalWordCount, WordCount, and
 * DebuggingWordCount examples: Reading text files; counting a PCollection;
 * writing to GCS; executing a Pipeline both locally and using a selected
 * runner; defining DoFns; user-defined PTransforms; defining PipelineOptions.
 *
 * <p>
 * New Concepts:
 * 
 ...
 *
 * <p>
 * By default, the examples will run with the {@code DirectRunner}. To change
 * the runner, specify:
 * 
 * <pre>
 * {@code
 ...
 * </pre>
 * 
 * See examples/java/README.md for instructions about how to configure different
 * runners.
 *
 * <p>
 ...
 *
 * <p>
 * By default, the pipeline will do fixed windowing, on 1-minute windows. You
...
public class WindowedWordCount
{
	 ...
	 * method, just for this example, for the bounded data case.
	 *
	 * <p>
	 * Imagine that many ghosts of Shakespeare are all typing madly at the same
	 * time to recreate his masterworks. Each line of the corpus will get a
	 * random associated timestamp somewhere in a 2-hour period.
	 */
	static class AddTimestampFn extends DoFn<String, String>
	{
		...
		public void processElement(ProcessContext c)
		{
			Instant randomTimestamp = new Instant(
					ThreadLocalRandom.current().nextLong(minTimestamp.getMillis(), maxTimestamp.getMillis()));
	...
	/**
	 * A {@link DefaultValueFactory} that returns the minimum timestamp plus one
	 * hour.
	 */
	 ...
	 * specification of the runner, as well as the
	 * {@link WordCount.WordCountOptions} support for specification of the input
	 * and output files.
	 */
	...
	public static void main(String[] args) throws IOException
	{
		...
		PCollection<String> input = pipeline
				...
				// time just to show windowing.
				// See AddTimestampFn for more detail on this.
				.apply(ParDo.of(new AddTimestampFn(minTimestamp, maxTimestamp)));

		/**
		 * Concept #3: Window into fixed windows. The fixed window size for this
		 * example defaults to 1 minute (you can change this with a command-line
		 * option). See the documentation for more information on how fixed
		 * windows work, and for information on the other types of windowing
		...
		/**
		 * Concept #4: Re-use our existing CountWords transform that does not
		 * have knowledge of windows over a PCollection containing windowed
		 * values.
		 */
		PCollection<KV<String, Long>> wordCounts = windowedWords.apply(new WordCount.CountWords());

		/**
		 * Concept #5: Format the results and write to a sharded file
		 * partitioned by window, using a simple ParDo operation. Because there
		 * may be failures followed by retries, the writes must be idempotent,
		 * but the details of writing to files is elided here.
		 */
		PCollection<String> formattedWords = wordCounts.apply(MapElements.via(new WordCount.FormatAsTextFn()));

		/**
		 * Combine into groups for batched write
		 */
		PCollection<KV<String, String>> firstLetterMap = formattedWords.apply(ParDo.of(new FirstLetterAsKey()));
		PCollection<KV<String, Iterable<String>>> groupedByFirstLetter = firstLetterMap
				.apply(GroupByKey.<String, String> create());
		PCollection<Iterable<String>> groupedWords = groupedByFirstLetter.apply(ParDo.of(new GetValues()));

		/**
		 * Write to Cloud Spanner through JDBC
		 */
		groupedWords.apply(JdbcIO.<String> writeIterable()
				.withDataSourceConfiguration(JdbcIO.DataSourceConfiguration.create("nl.topicus.jdbc.CloudSpannerDriver",
						"jdbc:cloudspanner://localhost;Project=numeric-wind-190510;Instance=test-instance;Database=test;PvtKeyPath=/home/loite/CloudSpannerKeys/cloudspanner-key.json"))
				.withStatement("insert into WordCount (word) values (?) on duplicate key update")
